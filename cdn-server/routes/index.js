...
let postDir = path.join(__dirname, '../avatars')
let urlavatar = `http://localhost:4000/avatars/`
let urlpost = `http://localhost:4000/posts/`
...
router.post('/upload_avatar', upload.single('avatar'), function (req, res, next) {
  try {
      if (!req.file) {
          ...
          let URL =urlavatar+ req.file.filename;
...
router.post('/upload_image', upload.single('imageProduct'), function (req, res, next) {
  try {
      if (!req.file) {
          throw new Error("k co file")
      } else {
          let URL =urlpost+ req.file.filename;
          CreateSuccessRes(res, {
              url: URL
          }, 200);
      }
...
router.get('/posts/:filename', function (req, res, next) {
  let pathFile = path.join(postDir, req.params.filename);
  res.sendFile(pathFile)
})
